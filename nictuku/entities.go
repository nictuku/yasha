		...
		parser.OnEntityPreserved = func(e *yasha.PacketEntity) {
			if e != nil {
				if e.Name == "DT_DOTA_PlayerResource" {
					...
					//jsonify(e)
					fmt.Println(e.Tick)
					fmt.Println("gold", e.Values["m_iTotalEarnedGold."+playersSuffix()[0]])
				}
				if e.Name == "DT_DOTA_DataRadiant" || e.Name == "DT_DOTA_DataDire" {
					fmt.Println("relgold", e.Values["m_iReliableGold.0005"])
					fmt.Println("unrelgold", e.Values["m_iUnreliableGold.0005"])
				}
			}
		}
		parser.Parse()
